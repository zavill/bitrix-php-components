...
else
{
    ...
    ?>
    <script src="https://api-maps.yandex.ru/2.1/?apikey=e7223c9e-a470-4407-b900-2477af924436&lang=ru_RU" type="text/javascript"></script>
    <div id="auth_registration_ordermake">
<!--        --><?//=(!$USER->IsAuthorized() ? '' : 'style="display: none;"')?>
        <div id="phone_auth" style="">
            <div id="auth_block_checkout">
                <h2>Вход или регистрация</h2>
                <div class="form_wrap">
                    <input type="text" placeholder="Телефон" value="+7" id="phone_input_checkout" class="login_input_first">
                    ...
                    <? if($arParams['ReCaptcha'] == 'Y'): $_SESSION['ReCaptchaAttempts'] = $arParams['ReCaptchaAttempts']; ?>
                        ...
                        <div id="g-recaptcha_checkout" class="g-recaptcha" data-sitekey="6LdxoPYUAAAAAIVEVdnzt-OQykRCjvfkUep1MdbY" <?=($_SESSION['SMS_COUNT_REQUEST'] >= $arParams['ReCaptchaAttempts'] ? '' : 'style="display: none;"')?>></div>
                    <? endif; ?>
                </div>
            </div>
            <div id="sms_code_block_checkout" style="display: none;">
                ...
                <div class="form_wrap">
                    <input type="text" maxlength="4" id="code_sms_checkout" class="login_input_first">
                    <div id="152fz_checkout" style="">
                        <input type="checkbox"> Нажимая кнопку "Отправить" Вы даёте свое согласие на обработку введенной персональной информации в соответствии с Федеральным Законом №152-ФЗ от 27.07.2006 "О персональных данных"
                    </div>
                    <p id="timer_again_checkout">Новый код можно получить через <span id="seconds_timer_checkout">60</span> секунд</p>
                    <p id="try_again_checkout" style="display: none">Попробовать ещё раз</p>
                </div>

            </div>
        </div>
        <div id="main_data_user" style="display: none">
            <h1>Ваши данные</h1>
            <p class="desc_info">Данные для оформления заказа</p>
            <div class="form_wrap">
                <input type="text" id="user_email" class="login_input_first" placeholder="E-mail">
                <input type="text" id="user_name" class="login_input_first" placeholder="Имя">
                <input type="text" id="user_surname" class="login_input_first" placeholder="Фамилия">
                <button id="send_user_info" class="form_login_button">Продолжить</button>
            </div>
        </div>
        <div id="user_adress" style="">
            <div id="header">
                <h1>Доставка</h1>
                <input type="text" id="suggest" class="login_input_first" placeholder="Введите адрес">
                <div class="adress_input_line">
                    <input type="text" id="apartament" class="login_input_first" placeholder="Квартира/офис">
                    <input type="text" id="postcode" class="login_input_first" placeholder="Индекс">
                </div>
                <div class="adress_input_line">
                    <input type="text" id="entrance" class="login_input_first" placeholder="Подъезд">
                    <input type="text" id="floor" class="login_input_first" placeholder="Этаж">
                    <input type="text" id="doorphone" class="login_input_first" placeholder="Домофон">
                </div>
                <input type="text" id="delivery_com" class="login_input_first" placeholder="Комментарий для курьера">
                <button type="submit" id="button" class="form_login_button">Продолжить</button>
                <p id="notice">Адрес не найден</p>
            </div>
            <div id="ya_map" style="width:100%; height:500px"></div>
        </div>
    </div>
    <form style="display: none" action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" class="bx-soa-wrapper mb-4<?=$themeClass?>" <?=($USER->IsAuthorized() ? '' : 'style="display: none;"')?> id="bx-soa-order-form" enctype="multipart/form-data">
        <?
        ...
        <div id="bx-soa-order" class="row" style="opacity: 0">
            ...
            <div class="col-lg-8 col-md-7 bx-soa">
                ...
                <!--	REGION BLOCK	-->
                <div id="bx-soa-region" data-visited="false" class="bx-soa-sectionbx-soa-section-hide">
                    <div class="bx-soa-section-title-container d-flex justify-content-between align-items-center flex-nowrap">
                        <div class="bx-soa-section-title" data-entity="section-title">
    ...
    if ($arParams['SHOW_PICKUP_MAP'] === 'Y' || $arParams['SHOW_MAP_IN_PROPS'] === 'Y')
    {
        ...
        if ($arParams['PICKUP_MAP_TYPE'] === 'google')
        {
            ...
            <script>
                ...
                {
                    if (BX.Sale && BX.Sale.BX.Sale.OrderAjaxComponentExt)
                        BX.Sale.BX.Sale.OrderAjaxComponentExt.initMaps();
                    else
        ...
        <script>
            (function bx_counter_waiter(i){
                ...
                if (typeof window['yaCounter<?=$arParams['YM_GOALS_COUNTER']?>'] !== 'undefined')
                    BX.Sale.BX.Sale.OrderAjaxComponentExt.reachGoal('initialization');
                else
